...
class Home extends Component {
    ...
    meteoroDaPaixao = async (evento) => {
        ...
        // this.setState({ seguidores: data })

        // const seguidores = await axios(`https://api.github.com/users/${input.value}/followers`)
        // this.setState({ seguidores: seguidores.data })

        // const informacoes = await axios(`https://api.bitbucket.org/2.0/users/${input.value}`)
        // this.setState({ informacoes: informacoes.data })
        // console.log(informacoes.data)

        const meteoro = await axios(`https://api.nasa.gov/planetary/apod?date=${input.value}&api_key=QVuTFtA9v3xTp70nC5FXTlVuHXfSkfcuShvqJ4LX`).catch()
            ...
            <>
                <Navbar color="dark">
                    <Container className="d-flex justify-content-center">
                        <img className="rounded-circle border border-white mr-3" width="50" src="https://www.thispersondoesnotexist.com/image" alt="pessoa aleatória" />
                        <span className="text-white">Logado como <Link className="text-white font-weight-bold ml-3">{this.props.match.params.usuario}</Link></span>
                    </Container>
                </Navbar>
                ...
                {this.state.meteoro.length === 0 && (
                    <Container className="h-100 d-flex flex-column justify-content-center align-items-center">
                        ...
                        <h3>Escolha uma data KKKKKKK</h3>
                    </Container>
                )}
